...
var shortform = {
    init: function () {
        ...
        {
            $(".shortApplicationForm select").select2({
                minimumResultsForSearch: Infinity
            });
        }


        $('body [rel="digits"]').keydown(function (e) {
        ...
        $('body [rel="dollars"]').keydown(function (e) {
            if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
                // Allow: Ctrl+A
                (e.keyCode == 65 && e.ctrlKey === true) ||
                // Allow: home, end, left, right
        ...
        $('body [rel="phone"]').keydown(function (e) {
            if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
                // Allow: Ctrl+A
                (e.keyCode == 65 && e.ctrlKey === true) ||
                // Allow: home, end, left, right
            ...
            $(this).val(phones($(this).val()));
            return;
        });

        $('body [rel="zip"]').keydown(function (e) {
            if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
                // Allow: Ctrl+A
                (e.keyCode == 65 && e.ctrlKey === true) ||
                // Allow: home, end, left, right
        ...
        $('.shortApplicationForm input[aria-required="true"], .shortApplicationForm select[aria-required="true"]').on("keyup change", function (e) {

            var isValid = ($(this).val().length > 0 && $(this).val() != " ");

            if (e.keyCode != 13) {

                if ($(this).hasClass("invalid") && isValid) {
                ...
                var elements = $(this).closest(".css-form-step");
                if (elements.find(".invalid").length == 0) {
        ...
        $('.shortApplicationForm input[type="text"]').on("keypress", function (e) {
            if (e.keyCode == 13) {
                var elements = $(this).closest(".css-form-step");
                elements.find(".js-actions").find(".css-button").last().trigger("click");
...
$(document).ready(function () {
    ...
    function validate(elements) {
        ...
        elements.find('input[aria-required="true"], select[aria-required="true"]').each(function () {
            ...
            if (value.attr("name").indexOf("email") > -1 && !validateEmail(value.val())) {
                response = response + 1;
            }
        });
        ...
        if (response > 0) {
            ...
            elements.find('input[aria-required="true"], select[aria-required="true"]').each(function () {
                var value = $(this);
                if (value.val().length == 0 || value.val() == " " || (value.attr("name").indexOf("email") > -1 && !validateEmail(value.val()))) {
    ...
    $('body').on('blur', '[name*="$LoanAmount"]', function () {
        if ($(this).val().replace("$", "") != "" && $(this).val() != $(this).attr('placeholder')) {
            if ($(this).closest('.mav-content').find('[name*="$LoanDownPayment"]').length > 0) {
                $(this).closest('.mav-content').find('[name*="$LoanDownPayment"]').attr('disabled', 'disabled').css({ opacity: 0.5 });
            }
        } else {
            if ($(this).closest('.mav-content').find('[name*="$LoanDownPayment"]').length > 0) {
                $(this).closest('.mav-content').find('[name*="$LoanDownPayment"]').removeAttr('disabled', 'disabled').css({ opacity: 1 });
            }
        }
    ...
    }).on('blur', '[name*="$LoanDownPayment"]', function () {
        if ($(this).val() != "" && $(this).val() != $(this).attr('placeholder')) {
            if ($(this).closest('.mav-content').find('[name*="$LoanAmount"]').length > 0) {
                $(this).closest('.mav-content').find('[name*="$LoanAmount"]').attr('disabled', 'disabled').css({ opacity: 0.5 });
            }
        } else {
            if ($(this).closest('.mav-content').find('[name*="$LoanAmount"]').length > 0) {
                $(this).closest('.mav-content').find('[name*="$LoanAmount"]').removeAttr('disabled', 'disabled').css({ opacity: 1 });
            }
        }
    });
    ...
    $('.shortApplicationForm .js-actions a').click(function () {

        var elements2 = $(this).children().attr('data-validate-part'),
            elements2 = $(elements2);

        var elements = $('.mav-content.active');


        var valid2 = validate(elements);
        console.log('validate', valid2);
        if (valid2) {
            $(this).children().attr('data-enabled', 'true');
            var enabled = $(this).children().attr('data-enabled');
            var direction = $(this).children().attr('data-direction');



            if ($(this).hasClass("css-submit-button")) {
                enabled = "false";
            }


            if (enabled == 'true') {
                var activeItem = $(this).closest('.mav-plug').find('.mav-content');
                var list = activeItem;
                var item = $(this).closest('.mav-content');
                var equiv = $(this).closest('.mav-content').index();
                var equiv = parseInt(equiv) - 1;
                var lis = $('.paginator li');

                var f = $(".shortApplicationForm .css-wrapper");
                f.addClass("noflow");
                ...
                if (direction == 'next') {
                    ...
                    /*
                    list.fadeOut(0);
                    lis.removeClass('passed');
                    //active this
                    item.next().fadeIn();
                    lis.eq(equiv + 2).addClass('passed');
                    */

                    list.addClass("fadeOut animated").removeClass("fadeOutRight fadeIn fadeInDown fadeOutDown fadeInLeft fadeInUp fadeInRight");
                    setTimeout(function () {
                        list.hide();

                        lis.removeClass('passed');
                        //active this
                        item.next().show().addClass("fadeIn animated").removeClass("fadeOutRight fadeOut fadeOutDown fadeInDown fadeOutUp fadeInLeft fadeOutLeft");
                        lis.eq(equiv + 2).addClass('passed');

                        setTimeout(function () {
                            f.removeClass("noflow");
                        }, time_one);
                    }, time_two);
                    ...
                    //active this
                    item.prev().fadeIn();
                    lis.eq(equiv - 1).addClass('passed');
                    */

                   


                    list.addClass("fadeOut animated").removeClass("fadeOutLeft fadeIn fadeInUp fadeOutUp fadeInLeft fadeInRight");
                    setTimeout(function () {
                        list.hide();

                        lis.removeClass('passed');
                        //active this
                        item.prev().show().addClass("fadeIn animated").removeClass("fadeOutRight fadeOut fadeOutDown fadeInUp fadeOutUp flipOutY fadeOutLeft fadeInRight");
                        lis.eq(equiv - 1).addClass('passed');
    ...
    $(".css-button.css-submit-button").on("click", function (e) {
        ...
        if (!validate(elements)) {
            ...
            // Start loading
            l.ladda('start');
            $("#page").css({ position: "relative", zIndex: 99999 });
        }
    });
    ...
    $dropdown_one.on("change", function () {
        ...
        if (current == "Refinance") {
            ...
            $drop.val("No Cash-Out").trigger("change");
            $drop.find("option").each(function () {
                if ($(this).val() == "No Cash-Out") {
